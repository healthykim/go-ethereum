...
func (r *resultStore) AddFetch(header *types.Header, snapSync bool, balBoundary uint64) (stale, throttled bool, item *fetchResult, err error) {
	r.lock.Lock()
	defer r.lock.Unlock()
	...
	if item == nil {
		item = newFetchResult(header, snapSync, balBoundary)
		r.items[index] = item
	}
	return stale, throttled, item, err
}
